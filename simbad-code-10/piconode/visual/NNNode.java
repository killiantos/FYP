...
public class NNNode extends NNShape{
	 ...
	 * @param x the x coordinate of the graphic representation
	 * @param y the y coordinate
	 * @param gridActivated indicates wether grid is set in NNCanvas.
	 */
	 ...
	 * @param node_arg the node to copy
	 */
	NNNode(NNNode node_arg){
		...
		if(undernode)
			underNodeDescription=node_arg.getUnderNode();
		nbNode++;
		myId=nbNode;
	...
	NNNode(String idnumber_arg,String name_arg,String type_arg,String function_arg,String x_arg,String y_arg
			,String red_arg,String green_arg,String blue_arg){
		...
		if (x_arg.equals(""))
			positionX=100;
		else positionX=Integer.parseInt(x_arg);
		
		if (y_arg.equals(""))
			positionY=100;
		else positionY=Integer.parseInt(y_arg);
	...
	/**
	 * delete a node
	 * @param list the list from wich the node has to be deleted
	 */
	 ...
	 * @param number : number zero will be placed in bottom case,number two in the third case etc... 
	 */
	...
	boolean isInRect(Rectangle rect){
		return (positionX>=rect.x && positionX<=rect.x+rect.width
	...
	{
		if ( (Math.pow( Math.abs(x_arg-positionX),2) + Math.pow( Math.abs(y_arg-positionY),2)) <= Math.pow( _RAYON,2))
			return true;
		return false;
	}
	...
	public void setType(String type) {
		//on decremente l'ancien type
		if(this.type.equalsIgnoreCase(types[1]))
			nbInputNode--;
		else if(this.type.equalsIgnoreCase(types[2]))
			nbOutputNode--;
		else if(this.type.equalsIgnoreCase(types[3]))
			nbHiddenNode--;
