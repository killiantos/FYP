...
public class Element_StaticArray_Double extends Element_Array {
	...
	/*public void performInitialisation() 
	{
		...
		this._array = new double[this._initGenotypeLength];
		for ( int i = 0 ; i != _initGenotypeLength ; i++ )
		{
			this._array[i] = ( Math.random() * 2 ) - 1;
		}
	}*/
	...
	public Object getArray () // !n - optimization note : byRef would speed up evolution while being less secured.
	{
		...
		//double [] doublearray = new double [this._array.length];
	...
	public void setArray ( Object __array ) // !n - optimization note : byRef would speed up evolution while being less secured.
	{
		try {
			...
			if ( this._array != null )
				if ( this._array.length != ((double[])__array).length )
